...
import Business_Layer.Transportations.Services.*;
...
public class Controler {
    ...
    public void Complete_stock_missing() {
        try {
            ...
            System.out.print(site_service.getSupplierByStoreArea(storeId, area));
            String[] suppliers = scan.nextLine().split(" ");
            boolean find_truck_driver = false;
            Date date = new Date();
            String driverId = "";
    ...
    public void Add_site() {
        ...
        String area = scan.nextLine();
        boolean result = site_service.addsite(type, name, city, street, number, name_of_contact, phone, area);
        if (result) {
    ...
    public void Remove_truck() {
        try {
            ...
            String id = scan.nextLine();
            trucks_service.removeTruck(Integer.parseInt(id));
