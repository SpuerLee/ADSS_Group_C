    ...
    public void updateRecordsSuppliers(HashMap<DummyItem, Integer> supply, Inventory inv, Service service) {
        ...
        for (DummyItem dummyItem : supply.keySet()) {
            ...
            if(records.containsKey(dummyItem.getId())) {
                ...
                newRecord = new Record(observers, id, dummyItem.getName(), dummyItem.getCost(), java.time.LocalDate.now(), java.time.LocalDate.now());
...
//                "type new price: ");
        ...
        List<Record> recList = records.get(id);
        Record lastRec = recList.get(recList.size()-1);
        Record newRecord = new Record(observers, id, nameLast, lastRec.getCost(), java.time.LocalDate.now(),
                dNewPrice , java.time.LocalDate.now());
        records.get(id).add(newRecord);
